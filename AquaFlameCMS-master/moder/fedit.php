    ...
    <div id="content">
      <div class="forms">
        ...
        <table width="100%" class="table-content">
          <tr>
            <td width="50%"><p><strong>Статус: </strong><?php if($forum['locked']==1) echo '<font color="red">Закрыт</font>'; else echo '<font color="green">Открыт</font>';?></p></td>
            <td>
              <p>
                ...
                <button type="submit" name="s_categ">Перенести</button>
              </p>
            </td>
          </tr>
          <tr><td>
          <div class="folder">
          <p><strong>Изображение: </strong><a href="javascript:;" onclick="pop('open');">
          ...
          <td><p><strong>Удалить форум с переносом тем в: </strong>
            <select name="f_delete" id="delete">
            <?php
              while($op = mysql_fetch_assoc($all_forum)){
                if($op['categ'] != $last_categ) echo '<optgroup label="Перенести в раздел '.$array_categ[$op['categ']].'">';
                echo'<option value="'.$op['id'].'" name="'.$op['name'].'" id="op_del'.$op['id'].'">'.$op['name'].'</option>';
                $last_categ = $op['categ'];
              }
              ?>
            </select> 
            <button type="submit" name="s_delete" onclick="return confirm_del();">Удалить</button>
          </p></td></tr>
          <tr><td><p><strong>Имя: </strong>
          <input type="text" name="f_name" value="<?php echo $forum['name']; ?>" class="reg" /></p></td>
          <td></td></tr>
          <tr><td><p><strong>Описание: </strong><br />
          <textarea name="f_desc" class="reg" style="width:450px;"/><?php echo $forum['description']; ?></textarea></p></td>
          <td></td></tr>
          <tr><td align="center"><p><button type="submit" name="save">Сохранить</button>
          <a href="forums.php?id=<?php echo $categ['id']; ?>"><button type="reset" name="cancel">Отмена</button></a></p></td>
          <td></td></tr>
        </table>
        </form></div>
    </div>
